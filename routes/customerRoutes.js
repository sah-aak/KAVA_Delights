...
router.get("/cart/add/:id/:qty/:fromCart", (req, res) => {
    ...
    Product.findById({ _id: productId }, (err, product) => {
        if(err) {
            console.log(err);
            //req.flash("error", "Το προϊόν δεν προστέθηκε στο Basket.");
            res.redirect('back');
        }
        cart.add(product, product._id, qty);
        req.session.cart = cart;
        //req.flash("success", "Το προϊόν προστέθηκε στο Basket επιτυχώς!");
...
router.get('/cart/reduce/:id', function (req, res, next) {
    ...
    Product.findById({ _id: productId }, (err, product) => {
        if(err) {
            console.log(err);
            //req.flash("error", "Το προϊόν δεν αφαιρέθηκε απο το Basket.");
            res.redirect('back');
        }
        cart.reduce(productId);
        req.session.cart = cart;
        //req.flash("success", "Το προϊόν αφαιρέθηκε απο το Basket επιτυχώς!");
